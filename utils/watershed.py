...
if __name__ == '__main__':
    ...
    gradx = cv.filter2D(dt, -1, kernelx)
    grady = cv.filter2D(dt, -1, kernely)
    # gradx = cv.Sobel(dt, cv.CV_8U, 0, 2, 5)
    # grady = cv.Sobel(dt, cv.CV_8U, 2, 0, 5)
    grad = np.sqrt(gradx ** 2 + grady ** 2).astype(np.uint8)
